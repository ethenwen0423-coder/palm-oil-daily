...
保存为 `reports/$REPORT_DATE.md`，一级标题控制在 15 个字以内。正文（不含消息来源链接表和固定 AI 声明）控制在 **2,400–3,200 字**。覆盖面用于建立完整研究坐标，核心观点仍只能由两个 Level 1 主驱动决定，禁止把栏目数量等同于驱动数量。

固定栏目顺序：

1. `【今日观点】`
   - 首屏 80–120 字给出 Top Call：最重要的一件事、基准方向、P/Y/OI 相对强弱、行动、失效条件和置信度。
   - 不堆价格，不重复交易表。
2. `【今日交易信号】`
   - 写综合评分、唯一策略方向和 P/Y/OI 强弱。
   - 用一张紧凑表格分别列出 P/Y/OI，表头固定包含：品种、方向、触发、确认、止损、目标、仓位上限、信号有效期；三行均不得空缺。
3. `【核心驱动与预期差】`
   - 只写两个主驱动；每个包含快照时间、为什么重要、传导链、已定价/未定价。
   - 写出最强反证及它如何推翻基准判断。
   - 新闻仅在这里完整出现一次，不再设置新闻摘要。
4. `【盘前市场全景】`
   - 用一张四列表格逐项覆盖八个研究维度：海外盘面、美豆与豆油、棕榈油产地、菜籽链、能源与生柴、国内现货与库存、合约结构与资金、天气物流与政策。
   - 表头固定为：维度、已验证事实、对 P/Y/OI 影响、盘中验证信号；八个维度各一行。至少五行必须同时给出精确数字和日期/快照时间；缺证据的维度写“证据缺口”，影响栏写“不计入方向”，不得用空泛判断补位。
   - 合约结构优先列 P/Y/OI rank=1 与 rank=2 的真实交割月合约、价格、涨跌和时点；rank=2 缺失时只在核验说明披露，不得补造。
   - 表后用一段 120–180 字研究归纳，明确“当前定价主线”“最大预期差”“盘中验证优先级”，不能逐行复述表格。
5. `【关键数据与价格】`
   - 10–14 项，必须含 P/Y/OI、可用的主次合约、关键外盘或原油、豆棕与菜豆油两个价差、至少两项产地/官方供需数据、至少一项国内仓单/库存/基差/利润证据和 P 关键位。
   - 使用“指标、数值、时点、含义”四列表格；每个数字带交易时段、统计日期或快照时间，并说明含义。
6. `【价格预测与验证】`
   - 用一张八列表格分别列 P/Y/OI，表头固定为：品种、参考价、基准判断、下沿观察、上沿观察、上修触发、下修/失效、置信度。
   - 参考价、方向、上下沿、触发与失效条件只允许复制结构化行情、既有策略结果和 generation feedback；不得自行计算日内概率或目标。
   - 明确这是盘前条件预测，不是确定性点位预测；置信度不得高于 generation feedback 上限。
   - 表格后紧邻写明：以上价格预测由 AI 基于所列来源和既定模型生成，不代表任何来源方的官方立场，不构成投资建议，用户须自行核验。
7. `【开盘推演】`
   - 使用“情景、触发、确认、动作、放弃条件”五列表格；高开、平开、低开三种情景均完整填写。
...
11. `【AI观点风险提示】`
   - 固定说明 AI 观点不代表任何来源方的官方立场、不构成投资建议或交易指令，用户须自行核验并按自身承受能力独立决策。
...
低于 92 分或出现以下任一情况均不得发布：关键行情/价位错误、主线使用陈旧 Level 2/3 信息、预测披露缺失、交易方向冲突、必需栏目或提纲缺失、首屏 Top Call 未同时写明基准方向/相对强弱/行动/失效条件/置信度、主驱动未明确排序为“主驱动一/主驱动二”、盘前全景未覆盖八个研究维度、量化证据少于五行、证据缺口被包装成方向、关键数据少于十项或缺少产地供需/国内现货证据、价格预测表缺少 P/Y/OI 或使用非结构化点位、开盘三情景或 Y/OI 对 P 的同步/背离处理缺失。不得以“栏目齐全、数字正确”代替研究判断。来源口径不同但可解释时可记 `WARN`，不得放过关键数字错误。
