(function () {
  ...
  function renderTrades(items) {
    ...
    el("trades-list").innerHTML = trades.map((item) => {
      ...
      const tradedAt = item.timestamp || item.time || item.date || "--";
      ...
      return `<article class="trade-card ${isExit ? "is-exit" : "is-entry"}"><header><div><span class="trade-variety">${escapeHtml(item.name || item.variety || "--")}</span><strong>${escapeHtml(item.contract || "合约待核验")}</strong></div><em class="trade-action-badge ${actionClass}">${escapeHtml(action)}</em></header><div class="trade-card-stats"><div class="trade-stat"><span>开平仓方向</span><strong>${escapeHtml(action)}</strong></div><div class="trade-stat"><span>成交手数</span><strong>${escapeHtml(item.quantity ?? "--")} 手</strong></div><div class="trade-stat"><span>成交价格</span><strong>${escapeHtml(priceText)}</strong></div>${pnlCell}</div><footer><span>虚拟成交</span><time datetime="${escapeHtml(item.timestamp || item.date || "")}">${escapeHtml(formatTime(tradedAt))}</time></footer></article>`;
    }).join("");
  }
