(function () {
  ...
  function renderTrades(items) {
    ...
    el("trades-list").innerHTML = trades.map((item) => {
      ...
      const productName = item.name || item.variety || "品种待核验";
      const productCodes = [...new Set([item.variety, item.contract].filter(Boolean).map((value) => String(value).toUpperCase()))];
      const pnlCell = isExit ? `<div class="trade-stat trade-pnl"><dt>平仓收益（含费）</dt><dd class="${pnl.className}">${escapeHtml(pnl.text)}</dd></div>` : "";
      return `<article class="trade-card ${isExit ? "is-exit" : "is-entry"}" aria-label="${escapeHtml(`${productName} ${action}成交`)}"><header><div class="trade-instrument"><span class="trade-field-label">品种</span><strong>${escapeHtml(productName)}</strong><span class="trade-contract">${escapeHtml(productCodes.join(" · ") || "合约待核验")}</span></div><em class="trade-action-badge ${actionClass}">${escapeHtml(action)}</em></header><dl class="trade-card-stats"><div class="trade-stat trade-stat-action"><dt>开平仓方向</dt><dd>${escapeHtml(action)}</dd></div><div class="trade-stat trade-stat-quantity"><dt>手数</dt><dd>${escapeHtml(item.quantity ?? "--")} 手</dd></div><div class="trade-stat trade-stat-price"><dt>价格</dt><dd>${escapeHtml(priceText)}</dd></div>${pnlCell}</dl><details class="trade-card-strategy"><summary>使用策略：${escapeHtml(strategyName(item))}</summary><span>${escapeHtml(strategyMeta(item))}</span><p>${escapeHtml(strategyRules(item))}</p></details><footer><span>虚拟成交</span><time datetime="${escapeHtml(item.timestamp || item.date || "")}">${escapeHtml(tradeTime(item))}</time></footer></article>`;
    }).join("");
  }
