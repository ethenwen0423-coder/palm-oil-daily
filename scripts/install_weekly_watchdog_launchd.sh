...
PROMPT='这是棕榈油周报的 macOS 系统级调度任务。当前任务每周日 21:15 主动生成，21:40 再补检一次。请先检查 '"$RUNTIME_ROOT"'/reports/当前上海日期-weekend.md、data/reports.js、downloads/当前上海日期-weekend.md 是否都存在，且正文不含“未实际调用”“当前环境未暴露调用入口”“这是测试报告”“排版调试样稿”。如果已经合格，立即结束，不要改文件。如果缺失或不合格，请使用当前上海日期生成或补跑周报：先 git pull --ff-only，然后运行 python3 scripts/run_financial_skills.py --date 当前上海日期 --kind weekend --timeout 90，读取 source_runs/当前日期-weekend/manifest.json 和 raw 文件，并运行 python3 skills/data_quality_gate_skill/scripts/validate_data.py --manifest source_runs/当前日期-weekend/manifest.json --strict，并读取 references/weekly_automation_prompt.md、references/wechat_oil_sources.md、skills/master_report_skill/SKILL.md、skills/data_quality_gate_skill/SKILL.md、skills/oil-report-freshness/SKILL.md、skills/report_writer_skill/SKILL.md、skills/vinson-research-writing/SKILL.md、skills/vinson-research-writing/checklist.md、skills/title-generation/SKILL.md、skills/title-quality-gate/SKILL.md；微信来源文件只提供历史样例来源池，不是固定引用清单，必须动态搜索本周最新同类微信/产业/期货公司来源进行观点补充，核心数据仍以交易所、官方机构、金融 skills 和研报为准。写正文和标题前必须先调用 master_report_skill，并按 market_data_skill、data_quality_gate_skill、oil_report_freshness、report_writer_skill、headline_skill、report_quality_gate 顺序调度；当前 market_data_skill 由 run_financial_skills.py 和 manifest/raw 结果承担，report_writer_skill 由 skills/report_writer_skill/SKILL.md 承担，并以 vinson-research-writing 作为通用写作规范，headline_skill 由 title-generation 与 title-quality-gate 承担，report_quality_gate 保留接口并由 data_quality_gate_skill、checklist、title gate、来源核验和 freshness 禁用项共同检查；不得跳过 oil-report-freshness，report_writer 和 title-generation 只能使用该 skill 输出的新增驱动和主线建议，不得把背景、旧政策、周度库存或待核验信息提升为本周/周一主线。按 report_writer_skill 要求把正文从资讯汇总升级为研究分析：每个核心观点必须回答为什么，按影响程度排序驱动，写清传导链，区分预期与现实，说明观点失效条件，并给出研究置信度；周报必须新增【本周新闻导向分析】，最多3条，把本周重要新闻写成方向、传导链、交易含义和置信度，不得写成新闻堆砌；不得为此拉长篇幅。按 Vinson Research Writing Standard、Title Generation 和 Title Quality Gate 提升可读性、机构研究风格和标题质量；标题必须先由 title-generation 基于 freshness 主线提炼市场主线并生成，再由 title-quality-gate 检查；未通过必须回到 title-generation 重写；标题/首页观点只能写市场观点和核心逻辑，具体价格、追高、低吸、止损、加减仓等执行动作只能放入周一开盘推演或交易计划；不得改变数据来源、业务逻辑或交易策略。报告结尾必须包含【消息来源链接】和【AI观点风险提示】，说明本报告仅代表 AI 基于公开信息和已调用数据源生成的研究判断，不构成投资建议。按既定周报规范生成 reports/当前日期-weekend.md，运行 bash scripts/deploy_report.sh 发布。报告必须覆盖棕榈油、豆油、菜油及原油、天气、政策等外部变量；标题日期后简介限制15字以内。最终简要说明是否生成、是否已发布。'
