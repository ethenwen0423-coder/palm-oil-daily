...
QUALITY="\$ROOT/data/report_quality/\$REPORT_ID.json"
...
if [[ -s "\$REPORT" && -s "\$DOWNLOAD" && -s "\$DATA" && -s "\$QUALITY" ]] \\
  ...
  && python3 -c 'import json,sys; value=json.load(open(sys.argv[1], encoding="utf-8")); raise SystemExit(0 if value.get("can_publish") is True and int(value.get("score", 0)) >= 92 else 2)' "\$QUALITY" \\
...
PROMPT='这是棕榈油周报的 macOS 系统级调度任务，周日21:15生成、21:40补检。先检查 reports/当前上海日期-weekend.md、data/reports.js、downloads/当前上海日期-weekend.md、data/report_quality/当前上海日期-weekend.json；四项均合格且质量分不低于92时按规范停止。需要生成或补跑时，必须先完整读取并严格执行 references/weekly_automation_prompt.md 及其列出的 skills；该文件和 skills/report_writer_skill/SKILL.md 是正文结构的唯一权威。正文严格按新版十栏顺序生成：【一句话核心观点】【本周验证与预期差】【核心数据变化】【下周主线与事件】【周一开盘推演】【交易计划】【风险提示】【信息来源与核验说明】【消息来源链接】【AI观点风险提示】。不得要求、恢复或额外新增旧独立栏目【本周三大变化】【本周新闻导向分析】【市场一致预期 VS 我的判断】【市场复盘】【下周重要事件】【一句话总结】。必须覆盖P/Y/OI和至少两个可核验相对价值指标；交易方向、概率与价位只取自既有数据和策略结果。先完成数据门禁、freshness治理、结构化提纲、正文、标题和92分报告审计，再运行 bash scripts/deploy_report.sh；不得修改日报内容复盘、预测评估、滚动指标或其调度。失败时重写或停止发布，不得绕过门禁。最终简要说明是否生成、审计是否通过、是否已发布。'
