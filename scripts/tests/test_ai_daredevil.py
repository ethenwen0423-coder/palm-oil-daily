...
class AiDaredevilTests(unittest.TestCase):
    def test_static_page_exposes_required_fund_sections_and_risk_notice(self):
        ...
        self.assertIn('assets/ai-daredevil.css?v=20260903-1', html)
        self.assertIn('assets/ai-daredevil.js?v=20260903-1', html)
        self.assertIn("名义金额 / 保证金", html)
        self.assertIn('id="position-trade-note"', html)
        self.assertIn("<th>今日动作</th><th>下一步指令</th>", html)
        self.assertIn("todayTradesForPosition", script)
        self.assertIn("todayActionMarkup(position, trades)", script)
        self.assertIn("item.timestamp || item.time", script)
        self.assertIn("今日已平仓", script)
        self.assertIn(".today-action-cell", stylesheet)
        self.assertIn("friendlyStrategyText", script)
        self.assertIn('replace(/current_bias/g, "当前方向")', script)
        self.assertIn('class="activity-item-meta"', script)
        self.assertIn('kind === "skipped" ? "未执行"', script)
        self.assertIn(".activity-grid > .panel { overflow: hidden; }", stylesheet)
        ...
        self.assertIn('class="trade-field-label">品种</span>', script)
        self.assertIn("开平仓方向", script)
        self.assertIn('<dt>手数</dt>', script)
        self.assertIn('<dt>价格</dt>', script)
        self.assertIn("平仓收益（含费）", script)
        self.assertIn('const pnlCell = isExit ?', script)
        self.assertIn('class="trade-stat trade-pnl"', script)
        self.assertIn('.trade-card.is-entry .trade-stat-price { grid-column: 1 / -1; }', stylesheet)
        self.assertIn('.trade-card-stats', stylesheet)
        self.assertIn('class="trade-card-strategy"', script)
        self.assertIn("使用策略：", script)
        self.assertIn(".trade-card-strategy", stylesheet)
    ...
    def test_installer_has_exact_session_open_and_hourly_schedule(self):
        ...
        self.assertIn('PURE_AI_FUND_PROCESS_TIMEOUT", "1800"', runtime)
