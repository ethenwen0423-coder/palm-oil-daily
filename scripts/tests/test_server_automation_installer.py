...
class ServerAutomationInstallerTests(unittest.TestCase):
    def test_installer_shell_syntax_and_fail_closed_apply_contract(self):
        ...
        self.assertIn('"run_market_collector.py --once-per-session"', script)
        self.assertIn("systemctl enable --now palm-oil-market-refresh.timer", script)
        ...
        self.assertIn("Scan institution and public research sources every five minutes", script)
        ...
        self.assertIn("systemctl enable --now palm-oil-ai-daredevil-quotes.timer", script)
        self.assertIn('"run_ai_daredevil_quotes.py"', script)
        self.assertIn("systemctl enable --now palm-oil-ai-daredevil-backtest.timer", script)
        self.assertIn("build_ai_daredevil_monthly_backtest.py", script)
        audit = (ROOT / "server" / "audit_runtime.py").read_text(encoding="utf-8")
        self.assertIn('"palm-oil-ai-daredevil-backtest.timer"', audit)
    ...
    def test_systemd_units_share_lock_and_only_write_scoped_runtime_paths(self):
        ...
        self.assertIn("06:30:00 Asia/Shanghai", installer)
        self.assertIn("06:50:00 Asia/Shanghai", installer)
        self.assertIn("11:35:00 Asia/Shanghai", installer)
        self.assertIn("15:05:00 Asia/Shanghai", installer)
        self.assertIn("21:20:00 Asia/Shanghai", installer)
        self.assertIn("23:10:00 Asia/Shanghai", installer)
        self.assertIn("02:40:00 Asia/Shanghai", installer)
        self.assertIn("03:00:00 Asia/Shanghai", installer)
        self.assertIn('"*-*-* *:2/5:00"', installer)
        self.assertIn('"*-*-* *:02/10:00"', installer)
        self.assertNotIn('"*-*-* *:07/20:00"', installer)
        self.assertIn("OnCalendar=Mon..Fri *-*-* 07:07:00 Asia/Shanghai", installer)
        self.assertIn("OnCalendar=Mon..Fri *-*-* 08:47:00 Asia/Shanghai", installer)
        self.assertIn("OnCalendar=Sun *-*-* 21:15:00 Asia/Shanghai", installer)
        self.assertIn("OnCalendar=Sun *-*-* 22:05:00 Asia/Shanghai", installer)
    ...
    def test_ai_timer_enablement_requires_chatgpt_codex_and_real_generation(self):
        ...
        self.assertIn("research_acceptance_now", script)
        self.assertIn('acceptance_date = trade_date', script)
        self.assertIn('--now "$acceptance_now"', script)
        self.assertIn('exit "$acceptance_status"', script)
        self.assertIn("require_current_acceptance", script)
        self.assertIn('payload.get("status") != "ok"', script)
        self.assertIn('payload.get("backend") != "codex-chatgpt-cli"', script)
        self.assertIn('payload.get("acceptance") != "real_model_report_draft_validated"', script)
