...
def source_payload() -> dict:
    return {
        ...
        "external": {
            ...
            "cbot_bean_oil": {"name": "CBOT豆油", "price": 70.0, "change_pct": 0.35},
            "cbot_soybean": {"name": "CBOT大豆", "price": 1314.5, "change_pct": 0.42},
        },
        "fundamental": {
            "spread": {
            ...
            "inventory": {
                "palm_oil_inventory": {"name": "棕榈油库存", "price": 120.0},
            },
            "official_supply_demand": {
                "latest_metrics": {
                    "production": {"label": "CPO产量", "value": 110.0},
                    "exports": {"label": "棕榈油出口", "value": 90.0},
                    "stocks": {"label": "期末库存", "value": 120.0},
                }
            },
        },
...
P相对强于Y与OI但仍属分化，只有三者同步并满足触发才执行；否则观望，不追价。
...
|---|---|---|---|
| 海外盘面 | 2026-07-24 17:59，FCPO报4723点 | P获成本支撑，Y/OI看共振 | FCPO能否延续强势 |
| 美豆与豆油 | 2026-07-24收盘，CBOT大豆1314.5、豆油70.0 | Y获外盘参照，P/OI看替代传导 | 美豆油是否强于大豆 |
| 棕榈油产地 | 2026-07-10发布，MPOB产量110、出口90 | 产量与出口共同约束P | 出口能否消化增产 |
| 菜籽链 | 证据缺口 | 不计入方向 | 等待ICE菜籽或国内菜系证据 |
| 能源与生柴 | 2026-07-27 08:22，WTI原油84.51美元/桶 | 生柴估值承压，三油上方受限 | 原油能否止跌 |
| 国内现货与库存 | 2026-07-10口径，棕榈油库存120 | P库存压力待需求消化 | 库存与基差是否同向改善 |
| 合约结构与资金 | 2026-07-27 08:22，P/Y/OI分别9515/8609/10191 | P相对强，板块仍分化 | 成交持仓是否同步放大 |
| 天气物流与政策 | 证据缺口 | 不计入方向 | 等待官方天气、物流或政策信息 |

当前定价主线是FCPO成本支撑与国内三油分化并存；最大预期差在于出口能否消化产量和库存压力；盘中验证优先级依次为P关键位、Y/OI共振、成交持仓与原油方向。
...
| MPOB产量 | 110吨 | 2026-06，2026-07-10发布 | 官方供给背景 |
| MPOB出口 | 90吨 | 2026-06，2026-07-10发布 | 官方需求背景 |
| 国内棕榈油库存 | 120吨 | 2026-07-10口径 | 国内实物压力 |
...
"""
    ...
    if count < 2450:
        ...
        report = report.replace("市场预期", filler * ((2450 - count) // len(filler) + 1) + "。市场预期")
    return report
...
class AuditReportTest(unittest.TestCase):
    def test_panorama_requires_real_rank_two_contract_when_available(self) -> None:
        ...
        audit._require_panorama_table(
            """| 维度 | 已验证事实 | 对P/Y/OI影响 | 盘中验证信号 |
|---|---|---|---|
| 海外盘面 | 2026-07-24收盘FCPO 4723 | P获支撑 | 外盘同向 |
| 美豆与豆油 | 2026-07-24收盘美豆1314.5、豆油70 | Y获支撑 | 豆油强于大豆 |
| 棕榈油产地 | 2026-07-10发布MPOB产量110 | P供给承压 | 出口消化 |
| 菜籽链 | 证据缺口 | 不计入方向 | 等ICE菜籽 |
| 能源与生柴 | 2026-07-27 08:22原油84.51 | 三油估值承压 | 原油止跌 |
| 国内现货与库存 | 2026-07-10库存120 | P承压 | 基差改善 |
| 合约结构与资金 | 2026-07-27 08:22，P2609报9515，P2701报9608 | P观察换月 | 主次同向 |
| 天气物流与政策 | 证据缺口 | 不计入方向 | 等官方信息 |

当前定价主线是成本支撑；最大预期差是出口变化；盘中验证优先级是P关键位、Y/OI共振与持仓。""",
            source,
            failures,
        )
...
| 能源与生柴 | 2026-07-27 08:22原油84.51 | 三油估值承压 | 原油止跌 |
| 国内现货与库存 | 2026-07-10库存120 | P承压 | 基差改善 |
| 合约结构与资金 | 2026-07-27 08:22，P2609报9515 | P观察换月 | 主次同向 |
| 天气物流与政策 | 证据缺口 | 不计入方向 | 等官方信息 |
...
| OI | 10191 | 观望 | 10000 | 10300 | 站稳上沿 | 跌破下沿 | ★★☆☆☆ |"""
    ...
    def test_valid_daily_report_passes_with_reproducible_numeric_sample(self) -> None:
        ...
        self.assertEqual(len(result["numeric_audit"]["sampled_noncritical"]), 7)
