...
负责：
- 先根据 market_data_skill、策略结果、forecast feedback 和 oil_report_freshness 生成内部结构化提纲
- 再按日报 1,000–1,400 字、周报 1,600–2,000 字预算生成正文
- 围绕唯一 Top Call、两个主驱动、传导链、预期差、最强反证、失效条件和研究置信度写作
- 完整保留策略结果中的触发、确认、止损、目标、仓位和信号有效期
不得负责：
- 生成 Headline
- 生成 Subheadline
- 计算或创造行情、交易价位、方向、仓位和概率
- 把低新鲜度信息重新提升为今日主线
- 新增大量一级标题或用重复资讯拉长正文
---
### 5. headline_skill
...
负责：
- 用 `report_writer_skill/scripts/audit_report.py` 执行一次只读高级编辑审计
- 检查标题、正文、来源、时效性、交易方向和预测披露是否一致
- 全量复核关键行情、涨跌幅、价差和交易价位，固定种子抽样至少三项其他数字
- 输出 100 分内部评分；低于 85 分或命中一票否决项时阻止发布
不得负责：
- 新增观点
- 伪造来源
- 覆盖前面 Skill 的职责
...
12. report_writer_skill 必须先写符合 schema 的提纲，再写正文；Writer 不得自行创造交易数字。
13. 关键行情错误、陈旧信息成为主线、预测披露缺失或交易方向冲突属于一票否决。
---
## 六、最终输出结构
日报最终结构：
1. Headline
...
周报使用 report_writer_skill 定义的十个紧凑栏目，不把供给、需求、周期研究框架扩写为额外客户可见栏目。
---
## 七、最终原则
