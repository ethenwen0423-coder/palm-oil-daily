...
- 场外结构建议页：`otc-structure.html`
  - 读取最新期货行情与技术位，依据《商品类产品化宣传单页-20241009》中的累进宝、采省易、期现易、惠鑫保、惠增收、价差宝、月差宝，以及用户提供的凤凰累计结构定义（2025-02-27）和气囊宝1.0结构定义（2025-06），匹配具体产品及询价参考点位。
  - 凤凰累计只在用户显式选择、业务节奏为逐日、目标为采购或库存套保且完成 R5 适当性确认后输出；低置信行情自动降至 1.0，5.0/6.0 仅展示结构定义、不自动推荐。
  - 气囊宝1.0支持正向/反向、安全垫、收益封顶、虚值行权和敲入建仓价优化；只在到期结算场景和路径风险确认后输出，低置信行情不输出参与率增强版本。
  - 所有推荐点位均从最新行情中的 MA20/MA60、20 日区间、统计通道、综合观察位与 ATR 动态选择；观察期按行情置信度和 ATR/价格比例生成实际起止日期，不沿用附件示例期限。
- 静态资源：`assets/`
  - 包含页面样式、交互逻辑和视觉资源。
- 数据文件：`data/`
